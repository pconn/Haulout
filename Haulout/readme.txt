...
from the 'summaryBy' statements and also by the HO_data_summary_hours.csv file). Note that plot creation
could potentially be moved to an .Rmd file if the pts_sp object in "Haulout_SPPtsDF_post_filtering.RData" were loaded).  The primar task of this script is 
to run GLMPM models, main output is the
model results (in the various "test_" .RData files).
